...
- **フレームワーク**: Ruby on Rails 7.2+ (Hotwire標準)
- **Ruby**: 3.3+
...
- Hotwire（Turbo + Stimulus）はRails 7標準で同梱
...
| `pdfplumber`相当 → `pdf-reader` | PDFテキスト抽出（登記簿パーサー） |
| `prawn` + `prawn-table` | PDF生成 |
| `leaflet-rails` or CDN | 国土地理院タイルによる地図表示 |
| `httparty` or `faraday` | 外部API呼び出し |
| `sidekiq` | バックグラウンドジョブ（PDF解析等） |
| `redis` | Sidekiqバックエンド + Turbo Streams |

### 0-3. Docker Compose
- PostgreSQL + PostGIS
- Redis（Sidekiq + ActionCable用）
...
- Devise による簡易認証（個人事業主1名想定だが将来拡張に備える）
...
- `app/services/registration_pdf_parser.rb` — ルールベースパーサー
  ...
  - 各セクション内を正規表現で構造化
- `app/jobs/parse_registration_job.rb` — Sidekiqジョブ
